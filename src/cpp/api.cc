...
NAN_METHOD(StartGame) {
  // coordinate game
  coordinator.LoadSettings(1, {});
  coordinator.SetParticipants(participants);

  // start game
  v8::String::Utf8Value mapPath(info[0]->ToString());
  coordinator.LaunchStarcraft();
  coordinator.StartGame((const char*)(*mapPath));
}
...
v8::Local<v8::String> nstr(char* str) {
...
NAN_METHOD(GetUnits) {
  ...
  for (int i = 0; i < units.size(); i++) {
    Nan::Set(a, i, nstr(TagToStr(units.at(i)->tag)));
  }
...
NAN_METHOD(CommandUnit) {
  ...
  if (info.Length() == 3) {
	  if (info[2]->IsString()) {
		  v8::String::Utf8Value targetStr(info[2]->ToString());
